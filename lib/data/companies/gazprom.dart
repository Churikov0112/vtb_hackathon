...
class Gazprom with ChangeNotifier {
  ...
  List<ChartSampleData> get stonksData {
    return [..._stonksData];
  }

  void rollTime(BuildContext context) {
    ChartSampleData oldestData = _stonksData[0];
    for (var i = 0; i < _stonksData.length - 1; i++) {
      _stonksData[i] = _stonksData[i + 1];
    }
    oldestData.x = Provider.of<Date>(context, listen: false).currentDate;
    _stonksData.last = oldestData;
    notifyListeners();
  }

  // процент роста с момента покупки акции
  double get percentOfIncreasing {
    return (stonksData.last != stonksData[stonksData.length - 2])
        ? 100 *
            (stonksData.last.open - stonksData[stonksData.length - 2].open) /
            stonksData[stonksData.length - 2].open
        : 0;
  }
}
